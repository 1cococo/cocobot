...
async def on_raw_reaction_add(payload):
    ...
    try:
        await add_activity(
            member,
            f"이모지 반응 {payload.emoji}"
        )
    except Exception as e:
        print(
            f"[ACTIVITY ERROR] 이모지 반응 처리 실패: "
            f"{member} / {e}"
        )
